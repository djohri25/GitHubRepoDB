/****** Object:  Procedure [dbo].[uspPrepareMaternityOutcomesLetters]    Committed by VersionSQL https://www.versionsql.com ******/

/*
11/1/2021		Sunil Nokku		Readuncommitted on MMF
1/13/2021		Sunil Nokku		Remove MVDIsNull Function
*/

CREATE PROCEDURE
[dbo].[uspPrepareMaternityOutcomesLetters]
(
...
BEGIN
	...
	DECLARE @v_procedure_name nvarchar(255) = 'uspPrepareMaternityOutcomesLetters';
	DECLARE @v_birth_threshold_weeks int = 2;
	DECLARE @v_birth_threshold_days int = @v_birth_threshold_weeks * 7;

	DECLARE @v_member_mailing nvarchar(255) = '%Outcomes%';

	DECLARE @v_user_name nvarchar(255);
	DECLARE @v_letter_name nvarchar(255) = 'Maternity Outcomes Survey';
...
-- Get the candidate list of packets
	...
	(
		...
		AND lm.Processed = 'N'
		WHERE
-- Trigger criteria: Contact Form/Maternity/Mom section: “Member needs” question = Outcomes Letter
