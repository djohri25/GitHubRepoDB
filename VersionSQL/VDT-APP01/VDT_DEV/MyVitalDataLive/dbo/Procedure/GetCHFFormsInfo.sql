/****** Object:  Procedure [dbo].[GetCHFFormsInfo]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[GetCHFFormsInfo]
...
BEGIN
	...
	@existPED bit,@existPEGA bit,@existOBIA bit,@existERIA bit,@existCMFW bit,@existPEGP bit,@existPEPP bit,@existAAF bit,@existACP bit,@existCHFCP bit,
...
@existGPEA bit, @existGPED bit,@existGPECHF bit,@existGPEGA bit, @existGPEGP bit,@existGPEPP bit, @existGACP bit
	...
	@existPEA = 0,@existCHF = 0,@showCHF = 0,@existGA=0,@existOBIA=0,@existCHF = 0,@existPED = 0,@existERIA=0,@existCMFW=0,@existPEGA = 0,@existPEGP = 0,@existPEPP = 0,
      @existAAF = 0,@existACP = 0,@existCHFCP = 0,
...
--FormCHFCPCarePlan
	if exists(select top 1 * FROM FormCHFCarePlan  where MVDID = @MVDID and CustID=@CustID)	
	begin
		set @existCHFCP = 1
	end

--General Form CHFCP Care Plan 
...
--END
	...
	select @age as 'age',@existsAAP as 'existsAAP', @showACT4_11yr as 'showACT4_11yr', 
		...
		@existAAF AS 'existAAF',@existACP AS 'existACP',@existCHFCP AS 'existCHFCP',
