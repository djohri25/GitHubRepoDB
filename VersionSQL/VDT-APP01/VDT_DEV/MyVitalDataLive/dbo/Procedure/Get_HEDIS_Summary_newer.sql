/****** Object:  Procedure [dbo].[Get_HEDIS_Summary_newer]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [dbo].[Get_HEDIS_Summary_newer]
	@Product int = 0,
	@CustID int,
	@TIN varchar(50) = 'ALL',
	@NPI varchar(50) = 'ALL',
...
BEGIN
	...
	DECLARE @Abbreviations TABLE (ID int Identity (1,1), Abbreviation varchar(50), testName varchar(100), isIncentive bit, GoalPerc DECIMAL(8,2))
	DECLARE @AbbreviationsFinal TABLE (ID int Identity (1,1), Abbreviation varchar(50))
	...
	SET @SQL =
		'SELECT a.Abbreviation, a.Name, b.isIncentive, b.Goal
	...
	IF (@SortBy IN ('testAbbr', 'testName', 'isIncentive', 'GoalPerc'))
	BEGIN
		;WITH PostCTE AS 
		(
				...
				(ORDER BY
					CASE WHEN @SortDirection = 'ASC' THEN
						CASE @SortBy
							...
							WHEN 'isIncentive' THEN CAST(isIncentive AS VARCHAR(50))
		...
		INSERT INTO @AbbreviationsFinal
		SELECT Abbreviation
		FROM PostCTE
	...
	BEGIN
		SELECT TOP 1  @ID = ID, @Abbreviation = Abbreviation FROM @AbbreviationsFinal
	...
	BEGIN
		...
		SELECT testID, testAbbr, testName, isIncentive, PrevYearPerc, CurYearToDatePerc, CurYearOverall, GoalPerc, QualifyingMemCount, CompletedMemCount, DueMemCount, YearToDateGoalStatus, CurYearOverallGoalStatus, MonthID
		FROM PostCTE
		WHERE RowNumber > ((@Page - 1) * ISNULL(@RecsPerPage, 0)) AND RowNumber <= (((@Page - 1) * ISNULL(@RecsPerPage, 0)) + ISNULL(@RecsPerPage, 100000))
	END
	ELSE
