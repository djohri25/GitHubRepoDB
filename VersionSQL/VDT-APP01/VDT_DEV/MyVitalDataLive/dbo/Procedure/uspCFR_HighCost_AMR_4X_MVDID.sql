/****** Object:  Procedure [dbo].[uspCFR_HighCost_AMR_4X_MVDID]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[uspCFR_HighCost_AMR_4X_MVDID] 
AS
/*
...
Scott   20210324	Add Company Exclusion for Tyson, Walmart, ASE, PSE
					and change name to usp_CFR_HighCost_AMR_4X_MVDID
Scott	20210525	Add Universal Exclusion for hourly and no benefit
...
EXEC uspCFR_HighCost_AMR_4X_MVDID   --(2,945,2:52) (2943/2:00)

EXEC uspCFR_Merge @MVDProcedureName = 'uspCFR_HighCost_AMR_4X_MVDID', @CustID = 16, @RuleID = 289, @ProductID = 2, @OwnerGroup = 159

EXEC uspCFR_MapRuleExclusion @RuleID = 289, @Action='ADD', @ExclusionID=21

*/
BEGIN
	SET NOCOUNT ON;

	DECLARE @RuleID int = 289
...
--New Exclusion Code
	...
	CREATE INDEX IX_ExcludedMVDID ON #ExcludedMVDID (MVDID)
		...
		SELECT DISTINCT a.MVDID
		  FROM AMR4 a
		 WHERE NOT EXISTS (SELECT 1 FROM #ExcludedMVDID WHERE MVDID = a.MVDID)
