/****** Object:  Procedure [dbo].[uspPrepareMaternityExchangeWelcomeLetters]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE
[dbo].[uspPrepareMaternityExchangeWelcomeLetters]
(
...
01/09/2020			Sunil Nokku				Add condition to use only those cases created when SectionCompleted (2,3)
10/29/2020			Sunil Nokku				TFS 3857
--EXEC uspPrepareMaternityExchangeWelcomeLetters null,null
1/11/2021		SunilNokku		Readuncommitted on MMF
*/
BEGIN
	SET NOCOUNT ON;

	DECLARE @v_procedure_name nvarchar(255) = 'uspPrepareMaternityExchangeWelcomeLetters';
	...
	DECLARE @v_letter_name nvarchar(255) = 'Maternity Exchange Welcome Letter';
...
-- Trigger criteria: Maternity Enrollment Assmt: completed/saved
		...
		(
			...
			AND ammf.SectionCompleted IN ( 2,3)
		) mmf
...
-- Trigger criteria: CM_ORG_REGION = EXCHNG
...
-- There is no Spanish language version of this form
...
-- Record that the letter was sent
	...
	--					OR ISNULL(MemberLastName, '') = ''
	--					OR ISNULL(City, '') = ''
	--					OR ISNULL(State, '') = ''
	--					OR ISNULL(Zipcode, '') = ''
	--					OR (ISNULL(Address1,'')='' AND ISNULL(Address2,'')='')
	--					) 
	--			  )
