/****** Object:  Procedure [dbo].[GetParklandSenseHealthSurveyFormsInfo]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[GetParklandSenseHealthSurveyFormsInfo]
...
BEGIN
	...
	@existPED bit,@existPEGA bit,@existOBIA bit,@existERIA bit,@existCMFW bit,@existPEGP bit,@existPEPP bit,@existAAF bit,@existACP bit,
	@existDCP bit,@existOCP bit,@existPHSF bit,
...
--@existGDCP bit
    ...
    @existAAF = 0,@existACP = 0,@existDCP = 0,@existOCP = 0,@existPHSF = 0,
...
END
	...
	--FormDiabetesCarePlan (changed from FormAddendum to FormDiabetesCarePlan same as FormAsthmaCarePlan)
	if exists(select top 1 * FROM FormDiabetesCarePlan  where MVDID = @MVDID and CustID=@CustID)	
	begin
		set @existDCP = 1
	end

	--FormObesityCarePlan (changed from FormAddendum to FormObesityCarePlan same as FormDiabetesCarePlan)
	if exists(select top 1 * FROM FormObesityCarePlan  where MVDID = @MVDID and CustID=@CustID)	
	begin
		set @existOCP = 1
	end

	--FormSenseHealthSurvey 
	if exists(select top 1 * FROM FormSenseHealthSurvey  where MVDID = @MVDID and CustID=@CustID)	
	begin
		set @existPHSF = 1
	end

--General Form Diabetes Care Plan 
...
--END
	...
	select @age as 'age',@existsAAP as 'existsAAP', @showACT4_11yr as 'showACT4_11yr', 
		...
		@existAAF AS 'existAAF',@existACP AS 'existACP',@existDCP AS 'existDCP',@existOCP AS 'existOCP',
		...
		@existGACP AS 'existGACP',@existPHSF AS 'existPHSF'
	--@existDCP AS 'existGDCP'
