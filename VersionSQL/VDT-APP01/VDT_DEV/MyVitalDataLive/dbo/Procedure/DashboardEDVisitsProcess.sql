/****** Object:  Procedure [dbo].[DashboardEDVisitsProcess]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author: Marc De Luca
-- Create date: 12/1/2017
-- Description:	Returns Monthly and YTD count of user logins
-- Example:	EXEC dbo.DashboardEDVisitsProcess @CustID = 10
-- =============================================
CREATE PROCEDURE [dbo].[DashboardEDVisitsProcess]
	@CustID INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT @MaxCreateDate = MAX(C.[Created]) 
		FROM dbo.EDVisitHistory C
	...
	SELECT 
		@CustID AS CustID
	,CAST(YEAR(S.[VisitDate]) AS CHAR(4)) + CASE WHEN LEN(MONTH(S.[VisitDate])) = 1 THEN '0'+ CAST(MONTH(S.[VisitDate]) AS CHAR(1)) ELSE CAST(MONTH(S.[VisitDate]) AS CHAR(2)) END AS MonthID
	,COUNT(*) AS MonthlyTotal
	FROM dbo.EDVisitHistory S
	JOIN dbo.Link_MemberId_MVD_Ins I ON S.ICENUMBER = I.MVDId
	WHERE I.Cust_ID = @CustID
	AND S.VisitDate >= @FirstMonth
	AND VisitType = 'ER'
	GROUP BY CAST(YEAR(S.[VisitDate]) AS CHAR(4)) + CASE WHEN LEN(MONTH(S.[VisitDate])) = 1 THEN '0'+ CAST(MONTH(S.[VisitDate]) AS CHAR(1)) ELSE CAST(MONTH(S.[VisitDate]) AS CHAR(2)) END
	...
	MERGE dbo.DashboardTotals AS target  
		...
		WHEN MATCHED THEN   
				UPDATE 
				SET  target.EDVisitsMonthlyTotal = Source.MonthlyTotal
						,target.EDVisitsYearlyTotal = Source.YTDTotal
						,target.DateModified = GETDATE()
	WHEN NOT MATCHED THEN  
			INSERT (CustID, MonthID, EDVisitsMonthlyTotal, EDVisitsYearlyTotal)  
