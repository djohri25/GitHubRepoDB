/****** Object:  Procedure [dbo].[SP_Search_MVD_Records_CustID]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[SP_Search_MVD_Records_CustID]
	@CustID int,
...
--			@Gender  as 'Gender', @DOB as 'DOB'
		...
		'select ICENUMBER,LASTNAME,FIRSTNAME,
			...
			(select top 1 isnull(Medicaid,'''') from mainInsurance i where i.icenumber = a.icenumber),
			(select top 1 isnull(MedicareNumber,'''') from mainInsurance i where i.icenumber = a.icenumber)
		...
		Delete #TEMPRESULT where
		MVDID not in (select a.MVDID from #TEMPRESULT a join Link_MemberId_MVD_Ins b
					on a.MVDID = b.MVDId where b.Cust_ID = @CustID)
