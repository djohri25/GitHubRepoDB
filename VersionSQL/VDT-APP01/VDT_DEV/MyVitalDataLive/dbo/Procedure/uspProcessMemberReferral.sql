/****** Object:  Procedure [dbo].[uspProcessMemberReferral]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE   procedure [dbo].[uspProcessMemberReferral] 
(
	@CustomerId int,
	@MemberId varchar(50),
	@Title nvarchar(100),
...
BEGIN TRY
	...
	If (@CheckAssignment =0)
		Begin
			if exists (select 1 from dbo.[ABCBS_MemberManagement_Form] (readuncommitted) where MVDID=@MVDID and CaseProgram=@CaseProgram 
				...
				Begin		
					...
					from dbo.[ABCBS_MemberManagement_Form] (readuncommitted)
			...
			If not exists (select 1 from dbo.[ABCBS_MemberManagement_Form] (readuncommitted) where MVDID=@MVDID and CaseProgram=@CaseProgram and InProgress='No' 
	...
	IF (@CHECKASSIGNMENT = 1 )
		begin
			...
			if exists (select 1 from dbo.[ABCBS_MemberManagement_Form] (readuncommitted) where MVDID=@MVDID and CaseProgram=@CaseProgram and InProgress='No'
			...
			If not exists (select 1 from dbo.[ABCBS_MemberManagement_Form] (readuncommitted) where MVDID=@MVDID and CaseProgram=@CaseProgram and InProgress='No'
				...
				Begin 
					...
					if exists (select 1 from Final_MemberOwner (readuncommitted) where MVDID = @MVDID and OwnerType = 'Primary' and IsDeactivated = 0)
						begin
							select top 1 @Nurse = OwnerName 
							from Final_MemberOwner (readuncommitted) 
							where MVDID = @MVDID 
