/****** Object:  Procedure [dbo].[CPConnectWithMMF_20200509]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[CPConnectWithMMF_20200509] 
...
BEGIN
	...
	BEGIN
		...
		SELECT @CarePlanID = CarePlanID, @cpMVDID = MVDID, @cpStatus = CarePlanStatus, @cpActivated = Activated, @cpProgramType = CarePlanType, @cpCaseID = CaseID 
		from MainCarePlanMemberIndex 
		where MVDID = @mmfMVDID and CarePlanStatus < 1 and Activated > 0 and UPPER(RTRIM(IsNull(CarePlanType,''))) = UPPER(RTRIM(@mmfProgramType)) and LEN(RTRIM(IsNull(CaseID,''))) < 1
