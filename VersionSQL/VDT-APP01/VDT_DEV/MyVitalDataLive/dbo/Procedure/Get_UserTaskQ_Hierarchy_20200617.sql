/****** Object:  Procedure [dbo].[Get_UserTaskQ_Hierarchy_20200617]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[Get_UserTaskQ_Hierarchy_20200617] 
	@CustomerId int,
	@ProductId int,
	@flg int = null,
	--@StatusId int = null,
...
lhaga.Group_ID
from
AspnetIdentity.dbo.AspnetUsers au
join Link_HpAlertGroupAgent lhaga
...
and au.UserName = @User;
			...
			CROSS APPLY
			(
				SELECT TOP 1 DueDate, StatusID, PriorityID, Owner, GroupID
			...
			(
				Act.Owner = @User
				OR EXISTS ( SELECT 1 FROM #GroupNames WHERE GroupID = act.GroupID )
			)
			AND EXISTS
			( SELECT 1 FROM #Status WHERE StatusID = act.StatusID )  --act.StatusId not in (select StatusId from #Status) 
			AND task.customerid=@CustomerId
