/****** Object:  Procedure [dbo].[uspPrepareMaternityPrenatalPackets]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE
[dbo].[uspPrepareMaternityPrenatalPackets]
(
...
BEGIN
	...
	DECLARE @v_procedure_name nvarchar(255) = 'uspPrepareMaternityPrenatalPackets';
	...
	DECLARE @v_file_name_template nvarchar(255) =
		CONCAT( 'Packets_', @v_short_code_vartext, '_PRENATAL_ccyymmdd_ccyymmdd.xlsx' );
	...
	DECLARE @v_packet_name nvarchar(255) = 'Prenatal Packet';
...
/*
There should be a column on this mail file to indicate if the SD group has the dental xtra coverage.
A Yes/No indicator. This data will be provided to VDT.  It is on our list of files to be sent to VDT.
*/
...
-- Trigger criteria: If a user completed the MMF to close a maternity enrollment case, member should no longer be identified on the weekly maternity pregnancy tips file.
		...
		*
		FROM
		(
-- Get members requested from external packet request
			SELECT
			fme.MVDID,
			NULL CaseOwner,
			NULL MMFID,
...
-- This situation should be picked up from the normal MEF flow, so it should be excluded
			WHEN amp.ID IS NULL AND dbo.Get_NumberOfDaysFromWD( mef.q3MemberGestation ) <= @v_gestation_threshold_days THEN 1
...
-- Exchange members should never be on the prenatal file.
	...
	ELSE 0
	END = 1
	INNER JOIN FinalEligibilityEtl fe
	ON fe.MVDID = fm.MVDID
	AND fe.MemberEffectiveDate <= getUTCDate()
	AND fe.MemberTerminationDate >= getUTCDate()
-- Check to see if packet has been sent
	...
	END = 1
	WHERE
-- Trigger criteria: Members should be on the Prenatal packet file if gestation at enrollment is 34 weeks or less.
-- Exception: If Gestation at time of enrollment is 35 weeks or greater, do not include on prenatal file.
	ISNULL( mef.gestationAtEnrollmentDays, 0 ) <= @v_gestation_threshold_days
