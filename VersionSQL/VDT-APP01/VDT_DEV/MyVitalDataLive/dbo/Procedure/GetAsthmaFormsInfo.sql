/****** Object:  Procedure [dbo].[GetAsthmaFormsInfo]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[GetAsthmaFormsInfo]
...
--FormAsthmaCarePlan
	if exists(select top 1 * FROM FormAsthmaCarePlan  where MVDID = @MVDID and CustID=@CustID)	--changed from FormAddendum to FormAsthmaCarePlan BW, 2/1/2016
	begin
		set @existACP = 1
	end

--General Form Asthma Care Plan --BW, 2/21/2016
...
END
	...
	select @age as 'age',@existsAAP as 'existsAAP', @showACT4_11yr as 'showACT4_11yr', 
		...
		@existERIA AS 'existERIA', @existCMFW AS 'existCMFW',@existPEGA AS 'existPEGA',@existPEGP AS 'existPEGP',@existPEPP AS 'existPEPP',
		 @existAAF AS 'existAAF',@existACP AS 'existACP',
