/****** Object:  Procedure [dbo].[Get_HEDIS_Summary_NPIs]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =========================================================
CREATE PROCEDURE [dbo].[Get_HEDIS_Summary_NPIs]
...
BEGIN
	...
	(EntityID VARCHAR(50), EntityName VARCHAR(250), GoalPerc DECIMAL(8,2), PrevYearPerc DECIMAL(8,2), CurYearToDatePerc DECIMAL(8,2), CurYearOverall DECIMAL(8,2), QualifyingMemCount INT, CompletedMemCount INT, DueMemCount INT, YearToDateGoalStatus INT, CurYearOverallGoalStatus INT, MonthID CHAR(6))
	...
	SELECT DISTINCT a.PCP_NPI AS EntityID,
					CASE
						WHEN b.[Provider Organization Name (Legal Business Name)] != '' THEN b.[Provider Organization Name (Legal Business Name)]
						ELSE dbo.fullName(b.[Provider Last Name (Legal Name)], b.[Provider First Name], '')
					END AS EntityName
	FROM [dbo].[Final_HEDIS_Member_FULL] a
		LEFT JOIN [dbo].[LookupNPI] b ON a.PCP_NPI = b.NPI
	...
	BEGIN
		...
		INSERT #TEMP_RESULTS
			EXEC [dbo].[Get_HEDIS_Summary_ByNPI] @Abbreviation = @Abbreviation, @CustID = @CustID, @LOB = @LOB, @TIN = @TIN, @NPI = @EntityID, @MonthID = @MonthID
	...
	BEGIN
		...
		SELECT EntityID, EntityName,  GoalPerc, PrevYearPerc, CurYearToDatePerc, CurYearOverall, QualifyingMemCount, CompletedMemCount, DueMemCount, YearToDateGoalStatus, CurYearOverallGoalStatus, MonthID
		FROM PostCTE
		WHERE RowNumber > ((@Page - 1) * ISNULL(@RecsPerPage, 0)) AND RowNumber <= (((@Page - 1) * ISNULL(@RecsPerPage, 0)) + ISNULL(@RecsPerPage, 100000))
	END
	ELSE
