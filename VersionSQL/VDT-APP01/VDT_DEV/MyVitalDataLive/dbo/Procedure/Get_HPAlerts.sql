/****** Object:  Procedure [dbo].[Get_HPAlerts]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[Get_HPAlerts2] (
--Declare
...
select @user2 = username from [AspNetIdentity].[dbo].AspNetUsers U where U.Id = @UserID
...
null as 	ChiefComplaint	,
null as 	EMSNote	,
case when Rg.UserName = @user2 then 'My Members' else Rg.UserName end as 	TriggerName	,
calc.memberid as MemberID, 
null as 	DateModified	,
null as 	ERVisitCount	,
null as 	PhysicianVisitCount	,
...
null as 	HedisDue	,
	...
	ISNULL(CONVERT(varchar,calc.dob,101),'') AS MemberDOB
		--main.DateOfBirth as MemberDOB,
		,case when Rg.UserName = @UserID then 'My Members' else Rg.UserName end as Text 
		--ins.PlanGroup as [Group],
		--ins.Region
		--,Calc.CaseID as CaseID
		--,isnull(calc.OpenCaseCount,0)           as NumOpenCases
		...
		--,isnull(Calc.CaseProgram,0)			 as CaseProgram
		--,isnull(Calc.MemberOwner,0)			 as MemberOwner
		from AssignedUsers RG
		inner join (select * from ComputedCareQueue where HealthPlanEmployeeFlag =0) calc 
		on calc.MVDID=rg.MVDID
		where RG.UserName = @User2 or RG.UserName in 
		(select G.[NAME] as queue  
from Link_HPAlertGroupAgent A
join HPAlertGroup G on G.ID = A.Group_ID
join [AspNetIdentity].[dbo].AspNetUsers U on U.Id = A.Agent_ID
where cust_id=16 and active=1 and U.Id = @UserID)
	end 