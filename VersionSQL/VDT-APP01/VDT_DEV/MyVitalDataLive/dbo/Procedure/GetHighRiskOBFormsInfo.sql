/****** Object:  Procedure [dbo].[GetHighRiskOBFormsInfo]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[GetHighRiskOBFormsInfo]
...
--@existGDCP bit
    ...
    @existAAF = 0,@existACP = 0,@existDCP = 0,@existHROBCP = 0,
...
END
	...
	--FormHighRiskOBCarePlan (changed from FormAddendum to FormHighRiskOBCarePlan same as FormHighRiskOBCarePlan)
	if exists(select top 1 * FROM FormHROBCarePlan  where MVDID = @MVDID and CustID=@CustID)	
	begin
		set @existHROBCP = 1
	end

--General Form HighRiskOB Care Plan 
...
--END
	...
	select @age as 'age',@existsAAP as 'existsAAP', @showACT4_11yr as 'showACT4_11yr', 
		...
		@existAAF AS 'existAAF',@existACP AS 'existACP',@existDCP AS 'existDCP',@existHROBCP AS 'existHROBCP',
