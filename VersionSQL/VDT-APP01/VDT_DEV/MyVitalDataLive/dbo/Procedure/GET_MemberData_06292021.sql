/****** Object:  Procedure [dbo].[GET_MemberData_06292021]    Committed by VersionSQL https://www.versionsql.com ******/
...
Craig	2021-06-09	Remove ValBasedProg as standalone field and concenate with BENI.
...
--Care Flow Rules (CFR)
		...
		DECLARE @BENI varchar(MAX) = ''
		 SELECT @BENI = '[' + IIF(me.MaternityElig = 'Yes','{"name":"Maternity"},','') 
		                    + IIF(md.DentalXtraInd = 'Y','{"name":"Dental Xtra"},','')
		 ...
		 LEFT OUTER JOIN VitalData_Maternity_DentalXtra md ON md.Member_ID = fm.MemberID
			  WHERE MVDID = @MVDID --'16AE4E39B125904C6770'
		...
		BEGIN
			...
			SELECT TOP (100)
				...
				,ccq.RiskGroupID AS ElixhauserScore
				,0 AS CharlsonScore
				...
				,isnull(@BENI, '') + isnull(fm.ValBasedProg, '') AS BENI
				,ccq.[BenefitGroup]		
			FROM #MPDS MPD
