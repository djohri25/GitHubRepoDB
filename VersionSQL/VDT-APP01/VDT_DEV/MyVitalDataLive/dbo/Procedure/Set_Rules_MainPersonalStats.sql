/****** Object:  Procedure [dbo].[Set_Rules_MainPersonalStats]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROC [dbo].[Set_Rules_MainPersonalStats]
(
	@CustID INT = NULL
)
AS
...
--SET @CustID = 11
	...
	SELECT Pvt.CustID as Cust_ID, Pvt.MVDID, Pvt.MemberID, Pvt.PCP_TIN, Pvt.PCP_NPI, Pvt.LOB, pvt.MonthID, 
	 ...
	 FROM (
	SELECT h.custID, h.MVDID, h.memberid, h.PCP_NPI, h.PCP_TIN, h.LOB , h.monthid, 1 as value, lh.Abbreviation 
	...
	where (h.CustID = @CustID or @CustID is NULL) and h.monthID = (Select MAX(h1.MonthID) from Final_HEDIS_Member_FULL h1 Where h1.CustID = h.CustID)
	) P
	...
	-- Select R.* 
	FROM #Temp_UPD_MainPersonalStats T JOIN Rules.MainPersonalStats R ON R.Cust_ID = T.Cust_ID and R.monthID = T.MonthID and R.MVDID = T.MVDID and R.MemberID = T.MemberID
	...
	where (h.CustID = @CustID or @CustID is NULL) and h.monthID = (Select MAX(h1.MonthID) from Final_HEDIS_Member_FULL h1 Where h1.CustID = h.CustID ) and R.ID is null
		  ) P
	pivot (max(p.value) for Abbreviation in 
		(
			W15,
