/****** Object:  Procedure [dbo].[Move_ActivatedRecords]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author:		
-- Create date: 9/13/2011
-- Description:	Move activated member records (insurance got renewed) from archive to main database
-- =============================================
CREATE Procedure [dbo].[Move_ActivatedRecords]
As
SET NOCOUNT ON
...
where Active = 1 AND isArchived = 1
...
BEGIN TRY	
	...
	begin
		...
		begin
			...
			BEGIN TRY
				...
				EXEC Move_ActivatedRecord_FromArchive
					@MVDID = @MVDID
			END TRY
			BEGIN CATCH
	...
	begin
		...
		EXEC HPM_Import.dbo.Upd_ImportSummary
			@ImportType = 'MoveBackFromArchive',
			@SuccessfulCount = @SuccessfulCount,
			@FailedCount = @FailedCount,
			@NewMemberCount = 0,
