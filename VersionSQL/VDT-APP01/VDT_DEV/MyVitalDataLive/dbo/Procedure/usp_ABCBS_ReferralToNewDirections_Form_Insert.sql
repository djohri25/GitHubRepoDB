/****** Object:  Procedure [dbo].[usp_ABCBS_ReferralToNewDirections_Form_Insert]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- Modified:			
-- 051320 DJ Modified sproc for changes related to 
-- TFS3043(task title,task owner, reminder date & due date)
-- ========================================================
CREATE PROCEDURE
[dbo].[usp_ABCBS_ReferralToNewDirections_Form_Insert]
(
	@p_PrintOnlyYN bit = 0
)
AS
BEGIN
	...
	DECLARE @v_task_narrative nvarchar(255) = 'Member has a new referral from New Directions.  Please review the ABCBS and New Directions Referral Form.';
	...
	DECLARE @v_case_program nvarchar(255) = 'Case Management';
...
-- Process the form
			...
			SET @v_task_title = case when @ReferralTo = 'ABCBS CM' then 'Referral from New Directions - ABCBS CM'
											   when @ReferralTo = 'ABCBS Dietitian' then 'Referral from New Directions - ABCBS Dietitian'
											   when @ReferralTo = 'ABCBS Pharmacist' then 'Referral from New Directions - ABCBS Pharmacist'
											   when @ReferralTo = 'ABCBS SW' then 'Referral from New Directions - ABCBS SW'
										  end;

            SET @v_task_owner = case when @ReferralTo = 'ABCBS CM' then 'Clinical Support'
											   when @ReferralTo = 'ABCBS Dietitian' then 'Dietitian'
											   when @ReferralTo = 'ABCBS Pharmacist' then 'Pharmacist'
											   when @ReferralTo = 'ABCBS SW' then 'Social Worker'
										  end;
...
-- Create the HP alert note
			...
			ELSE
			BEGIN
				PRINT CONCAT( 'About to HP alert note for MVDID = ', @v_mvdid, ' and ND form ID = ', @v_form_id, '.' );
			END;
