...
BEGIN
	...
	SET @v_sql =
		...
		(
			...
			'ParentTableLastModifiedColumnName', CHAR(10),
			'FROM', CHAR(10),
			@v_destination_server, '.', @v_batch_process_db, '.', @v_destination_schema, '.MVDMergeTableList', CHAR(10),
			'WHERE', CHAR(10),
			'DatabaseName = ''', REPLACE( REPLACE( @v_current_db, '[', '' ), ']', '' ), '''', CHAR(10),
			'AND MergeType != ''Disabled'';'
		);
...
-- Get the list of tables to merge
	...
	BEGIN
PRINT CONCAT( @v_table_name, ': ', current_timestamp );

		EXEC SetupMergeTableToReplica
			@p_DestinationServer = @v_destination_server,
			@p_DestinationDatabase = @v_destination_database,
			@p_DestinationSchema = @v_destination_schema,
