/****** Object:  Procedure [dbo].[DashboardDocumentsCreatedProcess]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author: Marc De Luca
-- Create date: 12/1/2017
-- Description:	Returns Monthly and YTD count of documents created
-- Example:	EXEC dbo.DashboardDocumentsCreatedProcess @CustID = 11
-- =============================================
CREATE PROCEDURE [dbo].[DashboardDocumentsCreatedProcess]
	@CustID INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT @MaxCreateDate = MAX(C.[DateCreated]) 
		...
		AND C.LinkedFormType IS NOT NULL
	...
	AND S.LinkedFormType IS NOT NULL
	...
	MERGE dbo.DashboardTotals AS target  
		...
		WHEN MATCHED THEN   
				UPDATE 
				SET  target.DocumentsCreatedMonthlyTotal = Source.MonthlyTotal
						,target.DocumentsCreatedYearlyTotal = Source.YTDTotal
						,target.DateModified = GETDATE()
	WHEN NOT MATCHED THEN  
			INSERT (CustID, MonthID, DocumentsCreatedMonthlyTotal, DocumentsCreatedYearlyTotal)  
