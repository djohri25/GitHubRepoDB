/****** Object:  Procedure [dbo].[DashboardUserLoginsProcess]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[DashboardUserLoginsProcess]
	@CustID INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	(
		...
		(
			...
			AND @CustID = 11
			UNION ALL
			SELECT L.CallDate AS LoginDate, L.UserID
...
ORDER BY YEAR(LoginDate), MONTH(LoginDate)
	...
	MERGE dbo.DashboardTotals AS target  
		...
		WHEN MATCHED THEN   
				UPDATE 
				SET target.UserLoginsMonthlyTotal = Source.MonthlyTotal
						,target.UserLoginsYearlyTotal = Source.YTDTotal
	WHEN NOT MATCHED THEN  
			INSERT (CustID, MonthID, UserLoginsMonthlyTotal, UserLoginsYearlyTotal)  
