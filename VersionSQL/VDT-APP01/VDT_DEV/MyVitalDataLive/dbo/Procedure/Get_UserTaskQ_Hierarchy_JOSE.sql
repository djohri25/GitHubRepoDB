/****** Object:  Procedure [dbo].[Get_UserTaskQ_Hierarchy_JOSE]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[Get_UserTaskQ_Hierarchy_JOSE] 
	@CustomerId int,
	@ProductId int,
	@flg int = null,
	--@StatusId int = null,
...
Ed/Jose/Sunil	2021-05-24	Added indexes on temp tables
							Changed WHERE clause to drive off of task.GroupID instead of from #GroupNames
...
--			(task.Owner = @v_user OR EXISTS ( SELECT 1 FROM #GroupNames WHERE GroupID = task.GroupID ))
			CASE
			WHEN task.Owner = @v_user THEN 1
--			WHEN gn.GroupID IS NOT NULL THEN 1
			WHEN task.GroupID IS NOT NULL THEN 1
			ELSE 0
			END = 1
