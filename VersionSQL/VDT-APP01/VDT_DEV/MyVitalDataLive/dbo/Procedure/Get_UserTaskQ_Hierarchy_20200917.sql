/****** Object:  Procedure [dbo].[Get_UserTaskQ_Hierarchy_20200917]    Committed by VersionSQL https://www.versionsql.com ******/
...
AS 
/*
--exec Get_UserTaskQ_Hierarchy @CustomerId=16, @ProductId=2, @User='ENWINSECK'
Modifications:
WHO		WHEN		WHAT
		...
		and au.UserName = @User;
			...
			INNER JOIN [dbo].ComputedCareQueue main
				...
				CASE -- Enforce privacy of employee members
					...
					ELSE 0
				END = 1
			WHERE (task.Owner = @User OR EXISTS ( SELECT 1 FROM #GroupNames WHERE GroupID = task.GroupID ))
