/****** Object:  Procedure [dbo].[uspCFR_HighCost_CHF_LO_MVDID]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[uspCFR_HighCost_CHF_LO_MVDID] 
AS
/*
    CustID:  16
    RuleID:  225
...
Changes
WHO		WHEN		WHAT
Scott	2020-11-02	ALTERd by refactor of original to call uspCFR_Merge
Scott	2021-04-22	Reformat to CTE form and add Universal Exclusions for no-benefit and hourly.
Scott	2021-09-07	Add query hints for Computed Care Queue

EXEC uspCFR_HighCost_CHF_LO_MVDID  --(    )

EXEC uspCFR_Merge @MVDProcedureName = 'uspCFR_HighCost_CHF_LO_MVDID', @CustID = 16, @RuleID = 225, @ProductID = 2, @OwnerGroup= 168

EXEC uspCFR_MapRuleExclusion @RuleID = '225', @Action = 'DISPLAY'

*/
BEGIN
	SET NOCOUNT ON;

	DECLARE @RuleID int = 225
