/****** Object:  Procedure [dbo].[uspTempMemberMerge]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE
[dbo].[uspTempMemberMerge]
(  
...
BEGIN 
	...
	FROM
	Final.dbo.FinalMember
	WHERE
	MemberID = @PermMemberID
	AND CustID = @CustID
	AND MVDID NOT LIKE '%TMP';

	IF ( @v_mvdid_temporary = @v_mvdid_permanent )
	BEGIN
		RETURN -1;
	END;
