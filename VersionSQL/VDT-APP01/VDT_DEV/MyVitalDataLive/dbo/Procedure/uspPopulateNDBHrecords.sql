/****** Object:  Procedure [dbo].[uspPopulateNDBHrecords]    Committed by VersionSQL https://www.versionsql.com ******/

/* =============================================
Author:	Deepank Johri
Create date: 08-12-2021
Description:	Populate New Directions data based on requirements from NDBH
Example: EXEC dbo.uspPopulateNDBHrecords 
...
============================================= */
CREATE PROCEDURE [dbo].[uspPopulateNDBHrecords]
...
FROM
	...
	ON Rx.MemberID = Ref1.MemID
	 ...
	 AND Ref1.LoadDate > GETDATE() - 1000
...
WHERE Ref.LoadDate > GETDATE() - 1000
AND (Ref.q2MemFirstName IS NOT NULL OR Ref.q2MemFirstName != '')
AND (Ref.q2MemLastName IS NOT NULL OR Ref.q2MemLastName != '')
AND (Ref.MemID IS NOT NULL OR Ref.MemID != '')
AND Ref.FormAuthor IS NOT NULL
 ...
 ************************/
		...
		AND Ref2.LoadDate > GETDATE() - 1000
...
--  OUTER APPLY
  ...
  WHERE 
  Ref.LoadDate > GETDATE() - 1000
  AND (Ref.q2MemFirstName IS NOT NULL OR Ref.q2MemFirstName != '')
  AND (Ref.q2MemLastName IS NOT NULL OR Ref.q2MemLastName != '')
  AND (Ref.MemID IS NOT NULL OR Ref.MemID != '')
  AND Ref.FormAuthor IS NOT NULL
  --and convert(date,ref.loaddate) = '2021-08-03'
--  and Ref.MVDID = '160DBAA10414F9BE8775'

  
END

--select distinct [q1RefFrom] FROM [dbo].[ABCBS_ReferraltoNewDirections_Form]