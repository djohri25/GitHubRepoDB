/****** Object:  Procedure [dbo].[uspPrepareMaternityOpportunityLetters]    Committed by VersionSQL https://www.versionsql.com ******/
...
*/
CREATE PROCEDURE
[dbo].[uspPrepareMaternityOpportunityLetters]
(
...
BEGIN
	...
	DECLARE @v_procedure_name nvarchar(255) = 'uspPrepareMaternityOpportunityLetters';
	DECLARE @v_birth_threshold_weeks int = 2;
	DECLARE @v_birth_threshold_days int = @v_birth_threshold_weeks * 7;

	DECLARE @v_member_mailing nvarchar(255) = '%Opportunity%';

	DECLARE @v_user_name nvarchar(255);
	DECLARE @v_letter_name nvarchar(255) = 'Maternity Opportunity Letter';
...
-- Get the candidate list of packets
	...
	(
		...
		AND lm.Processed = 'N'
		WHERE
-- Trigger criteria: Contact Form/Maternity/Mom section: “Member needs” question = Opportunity Letter
