/****** Object:  Procedure [dbo].[ProcessDischargeData]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author:		<Author,,Name>
-- Create date: <Create Date,,>
-- Description:	<Description,,>
-- 2017/06/09	Marc De Luca	Added update to eliminate dups from discharge data
-- =============================================



CREATE PROCEDURE [dbo].[ProcessDischargeData]
AS
BEGIN
	SET NOCOUNT ON;
	...
	-- Remove resubmitted records that have already been processed
	UPDATE h
	SET  h.IsProcessed = 1
		,h.ProcessNote = 'Pre-Ignore: Duplicate Record'
	FROM dbo.DISCHARGE_DATA_History h
	WHERE h.IsProcessed = 0
	AND EXISTS 
	(
		select 1 
		from dbo.DISCHARGE_DATA_History x 
		where x.IsProcessed = 1
		and x.MEDICAIDE_NUMBER = h.MEDICAIDE_NUMBER 
		and cast(x.ADMIT_DATE as date) = cast(h.ADMIT_DATE as date) 
	)
	...
	select 
		...
		p.FirstName,p.LastName,i.TerminationDate, h.Source, h.AdmitPlace
	...
	where IsProcessed = 0 and processAttemptCount = 0 and ISNULL(h.MEDICAIDE_NUMBER,'') <> ''
