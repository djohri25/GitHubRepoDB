/****** Object:  Procedure [dbo].[SP_Search_MVD_Records_For_LoggedInUser]    Committed by VersionSQL https://www.versionsql.com ******/
...
--		arguments based on logged-in user. The result is in XML format
-- =============================================
CREATE PROCEDURE [dbo].[SP_Search_MVD_Records_For_LoggedInUser]
...
--	@OutputLimit = 10
	...
	declare @MVDID varchar(20),
		...
		@hpId int				-- logged-in user's health plan id

	select @hpId = hpc.Cust_ID 
	from MDUser as mdu
...
--			@Gender  as 'Gender', @DOB as 'DOB'
		...
		--This will add filter patients based on logged-in user's health plan
		select @sql = CONCAT(@sql, 
								' and
								Exists(
								select top 1 mm.Id
								from MDUser_Member as mm
								where mm.CustID =',
								cast(@hpId as varchar),
								' and mm.MVDID = a.ICENUMBER)')
